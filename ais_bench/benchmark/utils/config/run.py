...
def get_models_attr(cfg):
    ...
    for model_cfg in cfg['models']:
        ...
        if attr not in ['local', 'service']:
            raise ConfigError(
                UTILS_CODES.ILLEGAL_MODEL_ATTR,
                f"Model config contain illegal attr, 'attr' in model config is '{model_cfg.get('attr')}', " +
                "only 'local' and 'service' are supported!"
            )
        if attr not in attr_list:
            attr_list.append(attr)

    if len(attr_list) != 1:
        raise ConfigError(UTILS_CODES.MIXED_MODEL_ATTRS, "Cannot run local and service model together! Please check 'attr' parameter of models")
