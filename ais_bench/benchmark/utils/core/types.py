...
def check_type_list(obj, typelist: List):
    ...
    expected_types = [_.__name__ if _ is not None else None for _ in typelist]
    actual_type = type(obj).__name__
    logger.warning(f"Type check failed: expected {expected_types}, got {actual_type}")
...
def check_dataset(obj) -> Union[Dataset, DatasetDict]:
    ...
    if isinstance(obj, Dataset) or isinstance(obj, DatasetDict):
        ...
        logger.warning(f"Dataset type check failed: expected Dataset/DatasetDict, got {type(obj).__name__}")
        raise AISBenchInvalidTypeException(
...
def check_list(obj) -> List:
    ...
    if isinstance(obj, List):
        ...
        logger.warning(f"List type check failed: got {type(obj).__name__}")
        raise AISBenchInvalidTypeException(
...
def check_str(obj) -> str:
    ...
    if isinstance(obj, str):
        ...
        logger.warning(f"String type check failed: got {type(obj).__name__}")
        raise AISBenchInvalidTypeException(
...
def check_dict(obj) -> Dict:
    ...
    if isinstance(obj, Dict):
        ...
        logger.warning(f"Dict type check failed: got {type(obj).__name__}")
        raise AISBenchInvalidTypeException(
...
def check_type(obj: Any, expected_type: Type[_T]) -> _T:
    ...
    if obj is None:
        if expected_type is type(None):
            return obj
        logger.warning(f"Got None when expecting {expected_type.__name__}")
        raise AISBenchInvalidTypeException(
            UTILS_CODES.INVALID_TYPE,
    ...
    logger.warning(f"Type check failed: expected {expected_type.__name__}, got {actual_type}")
    raise AISBenchInvalidTypeException(
...
def check_meta_json_dict(obj) -> Dict:
    ...
    def validate_recursive(data, valid_key_value_types):
        ...
        if extra_keys:
            logger.warning(f"Illegal keys found in meta JSON: {', '.join(extra_keys)}")
            raise ConfigError(
    ...
    if "request_count" in obj:
        if not _check_positive_int_value(obj["request_count"]):
            logger.warning(f"Invalid request_count value: {obj['request_count']}")
            raise ConfigError(
...
def check_output_config_from_meta_json(obj) -> bool:
    ...
    if not param:
        logger.warning("Missing 'params' in output_config")
        raise ConfigError(
    ...
    if method == "uniform":
        if "min_value" in param and "max_value" in param:
            if (not _check_positive_int_value(param["min_value"])) or (
                ...
                logger.warning(f"Invalid min_value or max_value: {param['min_value']}, {param['max_value']}")
                raise ConfigError(
            ...
            if int(param["min_value"]) > int(param["max_value"]):
                logger.warning(f"min_value ({param['min_value']}) > max_value ({param['max_value']})")
                raise ConfigError(
            ...
            logger.warning("Missing min_value or max_value for uniform distribution")
            raise ConfigError(
                UTILS_CODES.MISSING_PARAMS,
        ...
        if "percentage_distribute" not in param:
            logger.warning("Missing percentage_distribute parameter")
            raise ConfigError(
                UTILS_CODES.MISSING_PARAMS,
        ...
        if not _check_percentage_distribute(param["percentage_distribute"]):
            ...
            """
            raise ConfigError(UTILS_CODES.INVALID_PERCENTAGE_DISTRIBUTE, err_msg)
        logger.debug(f"Percentage distribution validated: {param['percentage_distribute']}")
        return True
    else:
        logger.warning(f"Unsupported distribution method: {method}")
        raise ConfigError(
