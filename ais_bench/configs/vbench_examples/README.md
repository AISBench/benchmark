...
AISBench **已适配 VBench 1.0**。本目录 [`ais_bench/configs/vbench_examples/`](.) 下放的是 **独立配置文件** 示例：在 **GPU** 或 **NPU** 上对已有生成视频做质量/语义类维度测评。**当前 aisbench 不包含多模态视频生成**，请完成视频生成后再跑测评。

## 目录

- [快速开始](#快速开始)
- [运行模式 `-m eval` 说明与限制](#运行模式--m-eval-说明与限制)
- [配置与输出](#配置与输出)
...
2. **（强烈推荐）预热本地缓存**
   VBench 会加载多种小模型权重；未缓存时可能长时间在线下载甚至失败。**一键下载脚本只读取 shell 里的 `VBENCH_CACHE_DIR`**，与配置文件顶层变量无关——若要和测评共用同一目录，请先在终端 `export VBENCH_CACHE_DIR=...`。细节见下文 [依赖与环境](#依赖与环境) 与 [vbench_cache_dependencies.md](vbench_cache_dependencies.md)。
...
3. **可选：在配置里指定缓存**
   在示例配置顶层设置 `VBENCH_CACHE_DIR = "/path/to/cache"`（或别名 `vbench_cache_dir`）可在测评子进程导入 vbench **之前**覆盖该进程环境变量；未设置时沿用 shell 中的 `export`。

4. **执行测评（推荐显式 `--mode eval`）**
...
（`-m eval` 与 `--mode eval` 等价，与同目录示例配置顶部注释写法一致。）

**设备**：运行时自动探测，NPU 可用则优先 NPU，否则使用 CUDA。

**命令行覆盖配置**：若 CLI 支持，可通过 `path=/your/video/dir` 等形式覆盖 `DATA_PATH`（以实际 `--help` 为准）。

### 与本节命令等价的方式

不写 `--mode eval`、直接 `ais_bench <config>.py` 也可跑通本目录示例；区别在于是否跳过主流程里的 Infer 等与 OpenICL 相关的阶段 — 详见下一节。

## 运行模式 `-m eval` 说明与限制

AISBench 中 `-m eval` 对应阶段为 **JudgeInfer → Eval → AccViz**，**不包含**主流程里的 **Infer**（OpenICL 推理）。

- **常规 OpenICL 任务**：`-m eval` 默认假设此前已通过 `all` / `infer` 或 `--reuse` 得到 `predictions/` 下的产物；否则通用评测链路往往无法正常消费结果。
- **本目录 VBench 示例**：各维度分数在 **Eval** 阶段由 `VBenchEvalTask` **直接读取视频目录**计算，**不依赖** `predictions/*.jsonl`。若数据集无 `judge_infer_cfg`，**JudgeInfer** 会整块跳过。此时 `ais_bench <config.py> --mode eval` 与默认 `all` 相比通常只是少跑一段占位 Infer；若 Infer 不可用，可优先使用 `--mode eval`。
- `-m eval` 仍会跑 **Eval** 与 **AccViz**；`-m viz` 仅汇总、不跑评测。
- CLI 帮助里的「evaluate existing inference results」主要针对通用流程；VBench 读视频算分是例外，**不必**先具备 predictions。
...
| 配置项 / 环境变量 | 作用 |
| --- | --- |
| `DATA_PATH` | 待测评视频根目录（**必填**），见 [eval_vbench_standard.py](eval_vbench_standard.py)、[eval_vbench_custom.py](eval_vbench_custom.py) |
...
| 配置名 | 说明 | 配置文件 |
| --- | --- | --- |
| eval_vbench_standard | 标准 prompt 测评，16 维；需提供视频目录与（可按需指定的）full_info json | [eval_vbench_standard.py](eval_vbench_standard.py) |
| eval_vbench_custom | 自定义输入（prompt 来自文件或文件名），10 维 | [eval_vbench_custom.py](eval_vbench_custom.py) |
...
- 评测逻辑封装在 **`ais_bench/third_party/vbench`**（VBench 1.0 接口）。
...
部分 torchvision 算子（如 nms、`roi_align`）在昇腾上可能仅 CPU，测评效率低。若 `torch < 2.7.1`，可参考 [昇腾适配 torchvision](https://gitcode.com/Ascend/vision) 安装对应版本以提速。

#### 各维模型权重

各维度所需模型与权重请按 VBench 官方说明自备。

#### 小模型依赖与本地缓存

CLIP、DINO、MUSIQ、RAFT、ViCLIP、GRiT、Tag2Text、BERT base 等小模型默认落在环境变量 **`VBENCH_CACHE_DIR`**（未设置则用 `~/.cache/vbench`）。

- 配置顶层可设 `VBENCH_CACHE_DIR`，在子进程导入 vbench 前生效。
- **`download_vbench_cache.sh` 只认当前 shell 的 `export`**；要与测评路径一致时请事先导出同名目录。完整清单与手动下载步骤见 [vbench_cache_dependencies.md](vbench_cache_dependencies.md)。
- 脚本会跳过已有文件，可重复执行。
- 若脚本失败或需部分手动下载，请查阅 [vbench_cache_dependencies.md](vbench_cache_dependencies.md)。
...
本节面向 **需要按官方方式自行生成评测视频** 的读者（与「仅用已有目录跑测评」的快速开始互不冲突）。
...
以 `DATA_PATH=/data/zhanggaohua/datasets/vbench/CogVideoX-5B-mini/` 为例，推荐组织为：
