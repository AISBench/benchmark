...
- **缓存根目录**：默认使用环境变量 `VBENCH_CACHE_DIR`，若未设置，则为 `~/.cache/vbench`。
- **一键下载脚本**：`scripts/download_vbench_cache.sh` 会自动按下述结构下载/准备资源。
...
```bash
bash scripts/download_vbench_cache.sh
```
...
当网络或权限问题导致 `scripts/download_vbench_cache.sh` 多次失败时，可以根据本节说明**手动下载每一份依赖并放到对应路径**，从而绕过一键脚本。
