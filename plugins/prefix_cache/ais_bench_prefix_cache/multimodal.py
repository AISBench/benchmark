...
from .multimodal_aisbench_config import DEFAULT_OUTPUT_TOKENS, build_aisbench_config
...
def run_multimodal_benchmark(
    *,
    ...
    for name in selected:
        ...
        # AISBench itself creates the timestamped execution directory below
        # this root.  Keep all multimodal scenarios under the same
        # ``outputs/default/<timestamp>`` layout; the scenario name is already
        # part of the dataset/model abbreviation used for the metrics files.
        scenario_root = root
        image_spec = manifest["datasets"][name]["image"]
        config_dir = scenario_root / "generated_config"
        config_dir.mkdir(parents=True, exist_ok=True)
        config_filename = (
            "multimodal_prefix_cache_perf.py"
            if len(selected) == 1
            else f"multimodal_prefix_cache_perf_{name}.py"
        )
