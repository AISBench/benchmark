...
```mermaid
flowchart TB
    A["scenario.json"] --> B["load_scenario"]
    B --> C{"manifest 存在?"}
...
1. `load_scenario` → 检查/生成产物 → `validate_artifacts` → 校验 `scenario_sha256`（防止复用旧配置产物）。
...
6. `render_aisbench_config` 设置环境变量、`exec` 用户 AISBench 配置、把类引用渲染为 import 别名，产出静态 `config.py`。
