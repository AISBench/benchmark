...
测试位于 `plugins/prefix_cache/tests/`，当前共 **31 个 test method**：`test_core.py`（17 个，纯逻辑/配置单元级）+ `test_pipeline.py`（10 个，离线 pipeline 集成级）+ `test_cli.py`（4 个，CLI/日志/进度级）。全部使用 FakeTokenizer 或 mock，**不加载真实 transformers、不访问网络、不连接 vLLM**。
...
- 当前本地回归发现 31 个 test method。审计中发现并修复了 `test_log_file_is_nested_under_log_directory` 的调用问题：时间戳原先被误传给 `_resolve_log_file` 的第三个位置参数（该位置实际是 `manifest_path`），现已改为关键字参数 `execution_timestamp=...`；修复后 31 项全部通过；
