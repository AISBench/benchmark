...
| `<run_id>.requests.jsonl` | 最小输入，固定 `question / answer`，可按 `output.output_key` 追加 `max_tokens` 或 `output_tokens` | 直接可压测的最简请求 |
...
- **最小请求输出字段** `output.output_key`：默认 `null`，requests 只含 `question/answer`；可选 `max_tokens` 或 `output_tokens`，值均来自内部 `max_tokens`。`full.jsonl.max_tokens` 始终保留，Dataset 从 full 读取在线生成长度；
...
- **请求顺序** `prefix_cache.order.strategy`：`sequential` / `within_group_shuffle` / `interleave` / `global_shuffle` / `input_len_asc`。其中 `cold + input_len_asc` 用于模拟无预热下从短请求到长请求逐步建立 Cache：生成、JSONL 行序、Dataset 对齐和 Inferencer 实际发送均保持每个 `(group_id, dp_rank)` lane 的顺序；同一 lane 前一请求完成后才放行下一条，不同独立 Cache 仍可并发；
...
测试位于 `plugins/prefix_cache/tests/`，当前标准库 unittest discovery 共执行 **144 项**，覆盖纯逻辑/配置、离线 pipeline、CLI/日志/进度、产物、指标、Inferencer 发送顺序与 mock 在线运行时。测试使用 FakeTokenizer 或 mock，**不加载真实 transformers、不访问网络、不连接 vLLM**。
...
| 求解器轨迹 | `test_cold_multi_dp_converges_after_each_lane_first_miss` | 多 DP 各 lane 首请求 miss 后，累计命中率单调收敛到目标 |

### 4.3 `test_pipeline.py` — 集成测试（11 个）

| 用例 | 验证点 |
|---|---|
| `test_four_artifacts_and_minimal_requests` | prepare 在 `result/` 产出四类文件；默认 requests 每行严格只含 `question/answer` 且字段顺序固定；`validate_artifacts` 通过 |
| `test_requests_optional_output_key_matches_extract_qa_semantics` | `output_key=max_tokens/output_tokens` 时按指定名字追加第三字段，值与 full 的 `max_tokens` 一致，产物校验通过 |
...
### 4.5 `test_inferencer.py` — 实际发送顺序测试（1 个）

| 用例 | 验证点 |
|---|---|
| `test_cold_lane_sends_short_to_long_despite_reverse_task_creation` | 故意按长→短逆序创建 cold 异步任务；`LaneSequencer` 仍确保同一 `(group, DP)` 的实际请求调用按 8→16→32 token 串行发生 |
...
| 无预热短→长逐步建 Cache | `test_groups_and_orders`、`test_cold_lane_sends_short_to_long_despite_reverse_task_creation` |
...
| 正式性能流式采集配置 | `test_model_config_enables_streaming_metrics`（验证 `build_model_config` 固定输出 `stream=True`） |

### 4.7 当前验证状态、边界与交付门禁

- 求解器正确性通过**穷举 oracle** 对照验证，是本分支质量的核心保证（覆盖 cold/warmup、单/多 lane、6 档目标值）；
- 当前本地标准库 unittest discovery 执行 144 项并全部通过，其中包含累计命中率轨迹、短→长实际发送顺序、可配置 requests 输出字段及字段顺序校验、流式性能采集配置、时间戳 Manifest、在线 runtime 和既有生成算法回归；
