...
测试位于 `plugins/prefix_cache/tests/`，当前标准库 unittest discovery 共执行 **140 项**，覆盖纯逻辑/配置、离线 pipeline、CLI/日志/进度、产物、指标、Inferencer 发送顺序与 mock 在线运行时。测试使用 FakeTokenizer 或 mock，**不加载真实 transformers、不访问网络、不连接 vLLM**。
...
- 当前本地标准库 unittest discovery 执行 140 项并全部通过，其中包含累计命中率轨迹、短→长实际发送顺序、可配置 requests 输出字段及字段顺序校验、时间戳 Manifest、在线 runtime 和既有生成算法回归；
