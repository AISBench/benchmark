...
`run` 的关键流程日志会同时写入 `output_dir_时间戳/log/<run_id_时间戳>.run.log` 和终端 stderr，stdout 仍只输出最终 analysis JSON。日志覆盖执行上下文、产物复用/自动 prepare、precheck、reset、每个 Group × DP warmup、baseline/after 指标、AISBench 静态配置与启动命令、KV 周期采样、每 DP query/hit 差值、全局命中率及告警。AISBench 子进程还会记录 Dataset 行合并、cold lane 等待/放行、逐请求 DP 路由和流式响应 chunk 摘要。日志只记录 Prompt 长度和 SHA-256，不打印 Prompt 正文、API key、Authorization Header 或原始请求体。
