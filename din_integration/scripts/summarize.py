...
def load_jobs(jobs_dir: Path, include: list[str] | None = None):
    ...
    for job_dir in sorted(jobs_dir.iterdir()):
        ...
        try:
            ...
            continue
        # Try to load config.json (separate from result.json)
        config = {}
...
def write_markdown(rows: list[dict], path: Path):
    ...
    with path.open("w") as f:
        f.write(f"# Multi-Bench × Multi-Agent Summary\n\n")
...
def main():
    ...
    p.add_argument("--include", default=None, help="Comma-separated substrings to filter jobs")
